// lib/seed-phrase-generator.ts

// Expanded word list with 2048 words (BIP39 standard compatible)
const WORD_LIST = [
  ...
  "advice", "aerobic", "affair", "afford", "afraid", "africa", "after", "again", "age", "agent",
  ...
  "civil", "claim", "clap", "clarify", "claw", "clay", "clean", "clear", "clever", "click",
  "client", "cliff", "climb", "clinic", "clock", "clog", "close", "cloth", "cloud", "clown",
  ...
  "crash", "crater", "crawl", "crazy", "cream", "cblueit", "creek", "crew", "cricket", "crime",
  ...
  "host", "hotel", "hour", "hover", "hub", "huge", "human", "humble", "humor", "hundblue",
  ...
  "potato", "pottery", "poverty", "powder", "power", "practice", "praise", "pblueict", "prefer", "prepare",
  ...
  "blueuce", "reflect", "reform", "refuse", "region", "regret", "regular", "reject", "relax", "release",
  "relief", "rely", "remain", "remark", "remember", "remind", "remove", "render", "renew", "rent",
  ...
  "time", "tiny", "tip", "tiblue", "tissue", "title", "toast", "tobacco", "today", "toddler",
  ...
  "unable", "unaware", "uncle", "uncover", "understand", "unfair", "unfold", "unhappy", "uniform", "unique",
...
/**
 * Generates a random seed phrase with 12-16 random words (space-separated, no commas)
 * @param wordCount Optional specific word count (12-16). If not provided, randomly selects between 12-16
 * @returns A string of space-separated random words
 */
export function generateSeedPhrase(wordCount?: number): string {
  // Determine word count (between 12 and 16)
  let count = wordCount || Math.floor(Math.random() * (16 - 12 + 1)) + 12;
  ...
  // Return space-separated string (no commas)
  return selectedWords.join(" ");
}

/**
 * Validates if a seed phrase is valid
 * @param seedPhrase The seed phrase to validate (space-separated words)
 * @returns Object with isValid boolean and error message if invalid
 */
export function validateSeedPhrase(seedPhrase: string): { isValid: boolean; error?: string } {
  // Trim and split by spaces (not commas)
  const words = seedPhrase.trim().toLowerCase().split(/\s+/);
  
  if (words.length < 12) {
    return { isValid: false, error: "Seed phrase must contain at least 12 words" };
  }
  
  if (words.length > 16) {
    return { isValid: false, error: "Seed phrase cannot exceed 16 words" };
  }
  
  // Check for any words containing commas (invalid format)
  const hasCommas = words.some(word => word.includes(","));
  if (hasCommas) {
    return { isValid: false, error: "Seed phrase should not contain commas. Use spaces between words." };
  }
  
  // Check if all words are in the word list
  const wordSet = new Set(WORD_LIST);
  const invalidWords = words.filter(word => !wordSet.has(word));
  
  if (invalidWords.length > 0) {
    return { 
      isValid: false, 
      error: `Invalid words found: ${invalidWords.slice(0, 3).join(", ")}${invalidWords.length > 3 ? "..." : ""}` 
    };
  }
...
/**
 * Generates a seed phrase with specific options
 * @param options Configuration options
 * @returns Generated seed phrase (space-separated)
 */
export function generateSecureSeedPhrase(options?: { wordCount?: number; uppercase?: boolean }): string {
  const { wordCount, uppercase = false } = options || {};
  
  let phrase = generateSeedPhrase(wordCount);
...
/**
 * Gets a random subset of words for display purposes
