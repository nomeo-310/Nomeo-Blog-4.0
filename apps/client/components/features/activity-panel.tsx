...
import { Bell, Users, X, Check, CheckCheck, UserCheck, UserX, ArrowRight, PenLine } from "lucide-react";
import { toast } from "sonner";
import { cn } from "@/lib/utils";
import { useActivityPanel } from "@/stores/activity-panel-store";
import { useNotifications, useConnectionRequests, useActivityCount, type NotificationItem, type ConnectionRequestItem } from "@/hooks/use-activity";
import { useCoAuthorInvites } from "@/hooks/use-coauthor-invites";

/**
 * ActivityPanel — slide-in panel (80% desktop / full-width mobile) holding the
 * Notifications and Connections tabs. Driven by the activity-panel store, so
 * the navbar bell (or anything else) just calls open().
 *
 * Mount this ONCE near the app root (e.g. in the root layout, after the navbar)
 * so it's globally available.
 */
export function ActivityPanel() {
  type ActivityPanelTab = "notifications" | "connections" | "coauthor";
      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed inset-y-0 right-0 z-[90] flex h-dvh w-full max-w-lg flex-col border-l border-border bg-background sm:w-[480px]"
          >
            {/* Header + tabs */}
            ...
            {/* Footer — links to the full notifications page */}
            {tab === "notifications" && (
...
function TabButton({ active, onClick, icon, label, count }: {
  ...
  return (
    <button onClick={onClick}
      ...
      <span className={cn("absolute inset-x-3 -bottom-px h-0.5 rounded-full bg-primary transition-opacity", active ? "opacity-100" : "opacity-0")} />
    </button>
  );
}
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
        {selecting ? (
          <>
            <button onClick={() => { setSelecting(false); setSelected(new Set()); }} className="text-sm text-muted-foreground hover:text-foreground">Cancel</button>
          ...
          <>
            <button onClick={() => setSelecting(true)} className="text-sm font-medium text-muted-foreground hover:text-foreground">Select</button>
      ...
      <ul className="divide-y divide-border">
        {notifications.map((n) => (
          <li key={n.id}>
            <button
              onClick={() => selecting ? toggle(n.id) : open(n)}
...
function ConnectionsTab() {
  ...
  const actAll = async (action: "accept" | "decline") => {
    try {
      ...
      toast.success(action === "accept" ? `Accepted ${count} request${count === 1 ? "" : "s"}.` : `Declined ${count} request${count === 1 ? "" : "s"}.`);
    } catch { toast.error("Something went wrong. Try again."); }
  };
  ...
  if (invites.length === 0)
    return (
      <EmptyState
        ...
        subtitle="Co-author invites from other creators will show here."
      />
    );
      ...
      {invites.map((inv) => (
        <li key={inv.postId} className="flex items-start gap-3 px-4 py-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2.5 flex gap-2">
              <button
                onClick={() => act(inv.postId, "accept", inv.postTitle)}
              ...
              <button
                onClick={() => act(inv.postId, "decline", inv.postTitle)}
                disabled={isResponding}
                className="inline-flex items-center gap-1.5 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-accent disabled:opacity-50"
              >
                Decline
