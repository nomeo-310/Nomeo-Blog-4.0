...
import { Search, FileText, User, Hash, ChevronRight, CornerDownLeft, X, TrendingUp, ChevronLeft, ArrowUp, Users, Loader2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { useSearch, type SearchType, type SearchResult } from "@/hooks/use-search";

/**
 * SearchPage — Google-style spotlight search for Nomeo.
 * Live, scoped (filter-before-search), React Query-cached search.
 */
...
export default function SearchPage() {
  ...
  const handleSubmit = () => {
    ...
    // Navigate to a full results page (build this route to render server-side
    // results). The inline dropdown is the quick-preview; this is "see all".
    // router is from next/navigation — import and use if you want navigation:
    //   router.push(`/search/results?q=${encodeURIComponent(query)}&type=${filter}`)
    window.location.assign(
      `/search/results?q=${encodeURIComponent(query.trim())}&type=${filter}`
    );
  };

  const getIcon = (type: string, active: boolean) => {
    const base = cn("h-4 w-4", active ? "text-current" : "text-muted-foreground");
    switch (type) {
      case "story": return <FileText className={base} />;
      case "author": return <User className={base} />;
      case "lounge": return <Users className={base} />;
      default: return <Hash className={base} />;
    }
  };
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-7xl flex flex-col items-center gap-4 md:gap-5 transition-all duration-500 ease-out">
        ...
        {/* Wordmark / heading — collapses away when searching */}
        <div
          className={cn(
        ...
        <div className="relative w-full max-w-3xl rounded-full bg-card border border-border shadow-sm transition-all focus-within:border-foreground/30">
          <div className="flex items-center px-4 py-2.5 md:px-5 md:py-3 gap-3">
            ...
            {/* Submit arrow — clear action for users who expect to press to search */}
            <button
              type="button"
        ...
        {/* ── Filter pills (appear when searching) ───────────────────── */}
        {isActive && (
          ...
          <div className="w-full rounded-2xl border border-border bg-card overflow-hidden transition-all duration-300 animate-in fade-in slide-in-from-top-2">
            {isError ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 h-auto min-h-[300px] max-h-[70vh] md:max-h-[480px] divide-y md:divide-y-0 md:divide-x divide-border relative">
                ...
                >
                  ...
                  {results.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <div className={cn("p-2 rounded-lg shrink-0 transition-colors", active ? "md:bg-white/15 bg-primary/10 text-primary md:text-inherit" : "bg-muted")}>
                          {getIcon(item.type, active)}
                        </div>

                        <div className="flex-1 min-w-0 pr-1">
                          <div className="flex items-center justify-between gap-2">
                            <span className={cn("text-sm font-semibold truncate tracking-tight", item.type === "tag" && "font-mono text-xs")}>
                          ...
                          <p className={cn("text-xs mt-0.5 truncate", active ? "md:text-primary-foreground/80 text-muted-foreground" : "text-muted-foreground")}>
                            {item.subtitle}
                          </p>
                        </div>
                ...
                {/* Preview / mobile drill-down */}
                {activeItem && (
                  <div
                    className={cn(
                      "md:col-span-6 bg-muted/20 p-4 md:p-5 flex flex-col justify-between overflow-y-auto custom-scrollbar h-full w-full",
                  ...
                  >
                    <div>
                      <button
                        ...
                        className="flex md:hidden items-center gap-1 text-xs font-semibold text-primary mb-4 active:opacity-70"
                      >
                      ...
                      <h3 className="text-base md:text-xl font-heading font-bold text-card-foreground mt-3 tracking-tight leading-snug">
                        {activeItem.title}
                      </h3>
                      <p className="text-xs text-muted-foreground mt-1">{activeItem.subtitle}</p>

                      <hr className="my-3 md:my-3.5 border-border" />

                      <p className="text-xs md:text-sm text-card-foreground/90 leading-relaxed whitespace-pre-line">
                        {activeItem.preview}
                      </p>
                    </div>
                    ...
                    <button
                      type="button"
                      className="mt-5 md:mt-6 w-full flex items-center justify-center gap-2 px-4 py-2 md:py-2.5 bg-primary hover:opacity-90 active:scale-[0.99] text-primary-foreground text-xs md:text-sm font-semibold rounded-lg transition-all"
                    >
                      {activeItem.type === "story" ? "Read full story" : activeItem.type === "author" ? "View profile" : activeItem.type === "lounge" ? "Enter lounge" : "Explore topic"}
