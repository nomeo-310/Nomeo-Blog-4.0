...
import { ArrowLeft02Icon, Tick02Icon, BookOpen02Icon,  Edit01Icon, Calendar02Icon, ArrowDown01Icon, Sparkles } from "@hugeicons/core-free-icons";
...
const CLOUDINARY_PRESET_PROFILE = "nomeo_blogs_profile";
...
export default function OnboardingModal({ isOpen, onComplete, topics, initialStepIndex = 0, defaults }: OnboardingModalProps) {
  ...
  return (
    <Modal
      ...
      maxHeight
    >
      <div className="relative grid h-[90vh] grid-cols-1 overflow-hidden rounded-2xl md:grid-cols-2">
        ...
        {/* ── Right panel ─────────────────────────────────────────── */}
        <div className="custom-scrollbar flex h-full flex-col overflow-y-auto rounded-2xl bg-card px-6 py-10 sm:px-10 md:rounded-l-none">
                  ...
                  {step === "intent" && (
                    <div className="grid gap-3">
                      <IntentCard
                        ...
                        icon={<HugeiconsIcon icon={BookOpen02Icon} className="h-5 w-5" />}
                  ...
                  {step === "done" && (
                    <div className="flex flex-col items-center py-6 text-center">
                      <span className="flex h-16 w-16 items-center justify-center rounded-full bg-primary/10">
                        <HugeiconsIcon icon={Tick02Icon} className="h-8 w-8 text-primary" />
                      </span>
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-20 mt-1 overflow-hidden rounded-lg border border-border bg-popover py-1 shadow-lg">
          {options.map((o) => (
            ...
            >
              {o.label}
              {o.value === value && <HugeiconsIcon icon={Tick02Icon} className="h-4 w-4 text-primary" />}
            </button>
          ))}
        </div>
      )}
