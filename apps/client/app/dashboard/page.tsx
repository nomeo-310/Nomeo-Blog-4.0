...
import { Files02Icon, UserMultiple02Icon, MoneyBag01Icon, Message01Icon, TradeUpIcon, ViewIcon, FavouriteIcon, Bookmark01Icon, ArrowRight02Icon, Edit01Icon } from "@hugeicons/core-free-icons";
    ...
    <DashboardLayout>
      <div className="space-y-8 max-w-4xl">
        ...
        {isCreator && (
          <>
            <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))" }}>
              <StatCard icon={<HugeiconsIcon icon={Files02Icon} className="h-5 w-5" />} label="Published posts" value={stats.postsCount} href="/dashboard/posts" />
              <StatCard icon={<HugeiconsIcon icon={ViewIcon} className="h-5 w-5" />} label="Total views" value={formatCount(stats.totalViews)} />
              <StatCard icon={<HugeiconsIcon icon={UserMultiple02Icon} className="h-5 w-5" />} label="Followers" value={formatCount(stats.followersCount)} href="/dashboard/subscribers" />
              <StatCard icon={<HugeiconsIcon icon={MoneyBag01Icon} className="h-5 w-5" />} label="Earnings (NGN)" value={formatMoney(stats.totalEarningsKobo)} href="/dashboard/earnings" />
            </div>
            ...
            {stats.loungeRequestsCount > 0 && (
              <div className="flex items-center justify-between rounded-xl border border-primary/20 bg-primary/5 px-5 py-4">
                ...
                <Link href="/dashboard/lounges" className="inline-flex items-center gap-1 text-sm font-semibold text-primary hover:underline">
                  Review <HugeiconsIcon icon={ArrowRight02Icon} className="h-3.5 w-3.5" />
                </Link>
              </div>
            )}
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                <HugeiconsIcon icon={Edit01Icon} className="h-4 w-4" /> Write a new post
              </Link>
              { stats.lounges < 1 &&
                <Link
                ...
                >
                  <HugeiconsIcon icon={Message01Icon} className="h-4 w-4" /> Create a lounge
                </Link>
              }
            </div>
          </>
        ...
        <div>
          ...
          <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))" }}>
            <StatCard icon={<HugeiconsIcon icon={Bookmark01Icon} className="h-5 w-5" />} label="Saved posts" value={stats.savedCount} href="/dashboard/saved" />
            <StatCard icon={<HugeiconsIcon icon={FavouriteIcon} className="h-5 w-5" />} label="Liked posts" value={stats.likedCount} href="/dashboard/liked" />
            <StatCard icon={<HugeiconsIcon icon={UserMultiple02Icon} className="h-5 w-5" />} label="Connections" value={stats.connectionsCount} href="/dashboard/connections" />
          </div>
        </div>
          ...
          <div className="rounded-2xl border border-primary/20 bg-primary/5 p-6">
            <div className="flex items-start gap-4">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary">
                <HugeiconsIcon icon={TradeUpIcon} className="h-5 w-5" />
              </span>
              <div>
                ...
                >
                  Upgrade your account <HugeiconsIcon icon={ArrowRight02Icon} className="h-3.5 w-3.5" />
                </Link>
              </div>
            </div>
          </div>
