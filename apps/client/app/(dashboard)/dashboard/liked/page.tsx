...
import { Heart, Clock, Lock, PenLine } from "lucide-react";
              ...
              <article key={p.id} className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all duration-200 hover:-translate-y-1 hover:border-primary/40 hover:shadow-md">
                ...
                <div className="flex flex-1 flex-col p-4">
                  ...
                  <div className="mt-auto flex flex-wrap items-center gap-3 pt-4 text-xs text-muted-foreground">
                    ...
                    {p.readingTime && <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" />{p.readingTime} min</span>}
                    <span className="inline-flex items-center gap-1"><Heart className="h-3 w-3 text-rose-400" />{p.likesCount.toLocaleString()}</span>
                  </div>
                </div>
              </article>
            ))}
