...
import { Bookmark, Clock, Eye, Lock, PenLine } from "lucide-react";
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {posts.length === 0 ? (
          <EmptyState
            icon={<Bookmark className="h-8 w-8 text-muted-foreground/30" />}
    ...
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all duration-200 hover:-translate-y-1 hover:border-primary/40 hover:shadow-md">
      <Link href={`/post/${post.slug}`} className="relative block aspect-[16/10] overflow-hidden bg-muted">
        {post.coverImage
          ...
          : <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-primary/10 to-primary/5"><PenLine className="h-7 w-7 text-primary/30" /></div>}
        {post.access === "paid" && (
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-auto flex flex-wrap items-center gap-3 pt-4 text-xs text-muted-foreground">
          ...
          {post.readingTime && <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" />{post.readingTime} min</span>}
          <span className="inline-flex items-center gap-1"><Eye className="h-3 w-3" />{post.viewsCount.toLocaleString()}</span>
        </div>
      </div>
    </article>
  );
